...
export const useStockfish = () => {
        ...
        worker.onmessage = (event: MessageEvent) => {
            ...
            if (line === 'uciok') {
                ...
                if (scoreMatch) {
                    ...
                    const currentEval = type === 'mate' ? `Mate in ${Math.abs(score)}` : (score / 100).toFixed(2);
    ...
    const analyzePosition = (fen: string, searchDepth: number = 15) => {
        if (engine && isReady) {
    ...
    return { isReady, bestMove, evaluation, depth, pv, analyzePosition, sendCommand };
};